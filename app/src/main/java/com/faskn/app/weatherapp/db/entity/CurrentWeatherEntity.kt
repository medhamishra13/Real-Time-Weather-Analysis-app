...
data class CurrentWeatherEntity(
    ...
    {
        return "Updated at: " + SimpleDateFormat("dd/MM/yyyy hh:mm a", Locale.ENGLISH).format(
            dt?.times(1000)?.let { Date(it) })
    }
    fun visibility():String
    {
        return "Visbility: " + (visibility?.div(1000)).toString().substringBefore(".")+"km"
    }
}
